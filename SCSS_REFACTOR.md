...
## Phase 2 extraction (historical)

The initial extraction produced a five-partial chain:
...
At that checkpoint, concatenating those partials after the first 20 lines of
the entrypoint reproduced the original 210,650-byte file exactly, including its
SHA-256 fingerprint. Quarto rendered all 40 pages successfully and all 20
Playwright comparisons passed.
...
3. `_02-editorial.scss`
4. `_03-overrides.scss` — late cascade/integration layer
5. `_10-navbar.scss`
6. `_11-footer.scss`
...
At the Phase 3 checkpoint, the entrypoint and five then-current partials
contained 4,753 SCSS lines, down from 7,583 lines at baseline (2,851 lines,
or 37.6%). This is a historical consolidation metric rather than a current
line count, because navbar, footer and page-shell ownership were subsequently
split into dedicated partials.

Selectors absent from static source may still be generated by Quarto,
