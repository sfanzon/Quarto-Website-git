...
- `astro/` — Astro main-site source and production static-site build
- `docs/` — legacy pre-rendered Quarto output (generated; not a production source)
...
The current hybrid build described below is a supported production
implementation. Native Astro project explainers are being evaluated, not
selected: a full-fidelity F1 proof of concept is the gate before any decision
to remove hybrid support. Independent Quarto resources deploy separately under
the unchanged URL/subdomain topology in `PUBLISHING_ARCHITECTURE.md`.
...
For normal local development after installation, run the hybrid watcher from
`astro/`:
...
Then use the intended edit → save → live preview workflow. The watcher rebuilds
the merged site when Astro or canonical Quarto project/note inputs change; normal
editing should not require running `quarto render` manually. For an isolated
Quarto-only diagnostic, the underlying commands remain available:
...
### Production static site (current implementation)
...
The deployable result is `astro/dist/`. Astro emits the shared navbar, footer
and stylesheet; Quarto project and note documents are rendered to a temporary
staging directory and merged afterwards. Existing document URLs remain unchanged.

For local development across both renderers, use `npm run dev:hybrid` from
`astro/`. It builds the local QA site once, serves `astro/dist/`, and rebuilds
automatically when its Astro or canonical Quarto project/note inputs change.
...
The automated build should run `npm run build:site` from `astro/`; no manual
rendering, copying or uploading should be required. The final deployable
artifact is `astro/dist/`.

### Content ownership

- **Astro** owns the site shell, navigation, homepage, catalogue/listing pages
  and structured website data.
- **Quarto** owns technical articles, mathematical documents and reproducible
  scientific content.
...
`docs/` is generated legacy output. Do not edit it directly. The production
deployment artifact is `astro/dist/`.
