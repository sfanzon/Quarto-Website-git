...
Most files under `includes/` are generated by `scripts/build-content.py` and should not be edited manually; the hand-written exceptions are `includes/site-footer.html`, `includes/project-navigation.html`, `includes/after-body.html`, and `includes/scroll-restoration-head.html`. `data/projects.generated.json` and `docs/` should also not be edited manually.
