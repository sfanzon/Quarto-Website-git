...
The production foundation builds Astro and all Quarto project pages into one
static tree without sharing an output directory during rendering:
...
and stylesheet; Quarto project documents are rendered to a temporary staging
directory and merged afterwards. Existing project URLs remain unchanged.
...
automatically when its Astro or canonical Quarto-project inputs change.
...
deployment target will be `astro/dist/` once deployment configuration is
migrated; that deployment change is intentionally outside this foundation.
